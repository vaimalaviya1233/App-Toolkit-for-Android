...
open class AboutViewModel : ScreenViewModel<UiAboutScreen , AboutEvents , AboutActions>(initialState = UiStateScreen(data = UiAboutScreen())) {
    ...
    private fun copyDeviceInfo() {
        viewModelScope.launch(context = Dispatchers.IO) {
            withContext(Dispatchers.Main) {
                screenState.updateData(newState = screenState.value.screenState) { current ->
                    current.copy(showDeviceInfoCopiedSnackbar = true)
                }
